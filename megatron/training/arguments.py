...
from megatron.core.transformer.enums import AttnBackend
from megatron.core.version_utils import (
    check_gaudi_version,
    is_habana_frameworks_min_version,
    is_ite_min_version,
)
...
def validate_args(args, defaults={}):
    ...
    map_dtype = lambda d: (
        eval(d)
        if not dtype_map.get(d, None) and isinstance(eval(d), torch.dtype)
        else d if isinstance(d, torch.dtype) else dtype_map[d]
    )
    ...
    if args.fp8_param_gather:
        assert (
            args.use_distributed_optimizer or args.use_torch_fsdp2
    ...
    if args.use_custom_fsdp:
        ...
        if args.data_parallel_sharding_strategy in ["optim_grads_params", "optim_grads"]:
            ...
            assert (
                args.gradient_accumulation_fusion is False
    ...
    if args.bf16:
        ...
        if cuda_available or is_accumulate_allreduce_grads_in_fp32_required:
            ...
            if args.rank == 0 and not args.accumulate_allreduce_grads_in_fp32:
                print(
                    'WARNING: accumulate and all-reduce gradients in bf16 for '
                    'bfloat16 data type.',
                    flush=True,
                )
    ...
    if args.moe_capacity_bins_num > 0:
        ...
        ), f'MoE Capacity Bins should be used with alltoall token disptacher, got {args.moe_token_dispatcher_type}'
    ...
    assert (
        not args.apply_norm_post_sub_block or not args.bias_dropout_fusion
    ...
    if args.num_experts is not None and not is_real_cuda_device_available() and is_lazy_mode():
        assert (
            args.num_experts < torch.iinfo(torch.int32).max
        ), "num_experts must be less than int32 max for lazy mode"
    ...
    if args.expert_model_parallel_size > 1:
        ...
        if args.rank == 0:
            print(
                "custom save and load distrib optimizer method is not yet supported for "
    ...
    if args.ckpt_format == "torch_dcp":
        ...
        assert (
            args.tensor_model_parallel_size <= 1 and args.encoder_tensor_model_parallel_size <= 1
        ), "--ckpt-format torch_dcp is not tested with megatron tensor parallelism."
        assert (
            args.pipeline_model_parallel_size <= 1
            and args.encoder_pipeline_model_parallel_size <= 1
    ...
    if cuda_available:
        ...
        if mp.context._default_context._default_context._name != "spawn":
            if args.rank == 0:
                print(
                    "WARNING: HPU doesn't support multiprocess using fork or forkserver, please use spawn for expected behavior"
                )
        os.environ["CUDA_DEVICE_MAX_CONNECTIONS"] = "1"
        args.gradient_accumulation_fusion = False
        args.masked_softmax_fusion = False
        args.no_persist_layer_norm = True
        if args.use_torch_compile and not args.fp8:
            args.fp8_coverage["mlp_row_parallel"] = False
        if args.moe_dynamic_hpu:
            if args.fp8 and not is_habana_frameworks_min_version("1.23.0"):
                raise RuntimeError("moe_dynamic_hpu fp8 requires Gaudi version >= 1.23.0")
            if not args.fp8 and not is_habana_frameworks_min_version("1.20.1"):
                raise RuntimeError("moe_dynamic_hpu requires Gaudi version >= 1.20.1")
            if args.rank == 0:
        ...
        skip_swiglu_check = args.moe_dynamic_hpu and args.fp8 and args.fp8_smooth_swiglu
        if not skip_swiglu_check:
            if is_habana_frameworks_min_version("1.21.0"):
                ...
                if (
                    not args.fp8_smooth_swiglu
                    and args.num_experts is not None
                    and args.fp8_coverage.get('mlp_row_parallel', True)
                ):
                    args.fp8_coverage["mlp_row_parallel"] = False
                    if args.rank == 0:
                        print("Disabled mlp_row_parallel in fp8_coverage for MoE")
            else:
                args.fp8_smooth_swiglu = False
                args.fp8_coverage["mlp_row_parallel"] = False
                ...
                if args.rank == 0:
                    print(
                        f"moe_grouped_gemm was disabled due to an insufficient Gaudi software version (>=1.21.0 required)."
                    )
                    print(
                        f"fp8_smooth_swiglu was disabled due to an insufficient Gaudi software version (>=1.21.0 required)."
                    )

        # CP version check:
        if args.context_parallel_size > 1:
            assert is_habana_frameworks_min_version(
                "1.21.0"
            ), f"Context Parallelism is supported with Gaudi software version 1.21.0 and above"
    ...
    if is_lazy_mode() and args.use_torch_compile:
        raise RuntimeError(
            'Cannot use lazy(PT_HPU_LAZY_MODE=1) and torch.compile(--use-torch-compile) modes together'
        )

    if is_lazy_mode() and args.use_torch_compiled_autograd:
        raise RuntimeError(
            'Cannot use lazy(PT_HPU_LAZY_MODE=1) and torch Compiled Autograd (--use-torch-compiled-autograd) modes together'
        )

    if not args.use_torch_compile and args.use_torch_compiled_autograd:
        raise RuntimeError(
            'Torch Compiled Autograd (--use-torch-compiled-autograd) enabled but without t.compile (--use-torch-compile)'
        )

    if args.use_regional_compilation and not args.use_torch_compile:
        raise RuntimeError(
            'Reginal compilation (--use-regional-compilation) enabled but without t.compile (--use-torch-compile)'
        )

    if not args.use_torch_compile and args.compile_from_sec_mini_batch:
        raise RuntimeError(
            'Compile From Second Iteration (--compile-from-sec-mini-batch) enabled but without t.compile (--use-torch-compile)'
        )

    # Custom op checks
    def maybe_set_custom_op_flags(args):
        if args.use_torch_compile and args.fp8 is not None and args.use_te_custom_op:
            args.allow_unspec_int_on_nn_module = is_torch_min_version("2.8.0")
            custom_op_supported_framework_version = "1.23.0"
            if is_habana_frameworks_min_version(custom_op_supported_framework_version):
                os.environ["PT_TE_CUSTOM_OP"] = "1"
                args.use_te_custom_op = True
            else:
                print(f'Warning: Framework version is below {custom_op_supported_framework_version}, disabling custom op flag')
                os.environ["PT_TE_CUSTOM_OP"] = "0"
                args.use_te_custom_op = False
        else:
            os.environ["PT_TE_CUSTOM_OP"] = "0"


    maybe_set_custom_op_flags(args)

    # WA for SW-231982
    if is_custom_op():
        if args.num_experts is not None and args.use_torch_compile:
            assert is_ite_min_version(
                "1.23.0.137"
            ), f"Mixtral with custom op requires Intel Transforme Engine version 1.23.0.137 or above"
        else:
            assert is_habana_frameworks_min_version(
                "1.22.0"
            ), f"custom op solution is supported with Gaudi software version 1.22.0 and above"
            if not is_habana_frameworks_min_version("1.23.0"):
                assert (
                    args.pipeline_model_parallel_size == 1
    ...
    if args.inference_dynamic_batching:
        ...
        assert (
            args.inference_dynamic_batching_chunk_size % 256 == 0
        ), "chunk size should be a multiple of 256"
    ...
    if args.moe_capacity_bins_num != 0:
        ...
        assert (
            args.moe_token_dispatcher_type == "alltoall"
        ), 'moe_capacity_bins require alltoall token dispatcher'
    ...
    if args.mtp_num_layers:
        ...
        ), "Multi-Token Prediction (MTP) is not supported with Context Parallelism."
...
def core_transformer_config_from_args(args, config_class=None):
    ...
    if len(args.cp_comm_type) == 1:
        ...
        if kw_args['cp_comm_type'] == "a2a" and not is_real_cuda_device_available():
            assert is_ite_min_version(
                "1.23.0.220"
            ), f"Context Parallelism with cp_comm_type=a2a is supported with Gaudi software version 1.23.0.220 and above"
...
def _add_transformer_engine_args(parser):
    ...
    group.add_argument('--use-te-custom-op',
        type=lambda x: x.lower() in ['true', '1'],
        default=True,
        help='Use custom op for TE')
...
def _add_training_args(parser):
    ...
    group.add_argument(
        ...
        type=str,
        default=None,
        help='Path to look for a kill switch. ' 'If found will automatically exit the program.',
    )
    group.add_argument(
...
def _add_distributed_args(parser):
    ...
    group.add_argument(
        '--use-custom-fsdp', action='store_true', help='Use the Megatron FSDP code path in DDP.'
    )
    ...
    group.add_argument(
        '--keep-fp8-transpose-cache-when-using-custom-fsdp',
        action='store_true',
...
def _add_moe_args(parser):
    ...
    group.add_argument(
        '--moe-token-distribution-logging-interval',
        type=int,
        default=50,
...
def _add_pytorch_args(parser):
    ...
    group.add_argument(
        ...
        type=lambda x: (
            None if x is not None and x.lower() == 'none' else str(x).lower() in ['true', '1']
        ),
        default=False,
        help='Dynamic shape tracing value for torch.compile (can be True, False or None)',
    )
    group.add_argument(
        '--use-torch-compiled-autograd',
        type=lambda x: x.lower() in ['true', '1'],
        default=False,
    ...
    group.add_argument(
        '--use-regional-compilation',
        type=lambda x: x.lower() in ['true', '1'],
        default=False,
        help='Enable regional compilation',
    )
    group.add_argument(
        '--compile-from-sec-mini-batch',
        type=lambda x: x.lower() in ['true', '1'],
        default=False,
        help='Enable compile from second mini batch',
    )
    group.add_argument(
        '--allow-unspec-int-on-nn-module',
        type=lambda x: x.lower() in ['true', '1'],
        default=False,
        help='allow int members to be potentially unspecialized' 'through dynamic shape mechanism',
    )
    return parser

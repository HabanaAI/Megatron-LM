            ...
            if is_lazy_mode():
                ...
                for name, param in model_module.named_parameters():
                    ...
                    for attr_name, attr_value in param.__dict__.items():
                        param_attr[name][attr_name] = attr_value
            model_module.cuda(torch.cuda.current_device())
            if is_lazy_mode():
                for name, param in model_module.named_parameters():
                    if name in param_attr:
                        for attr_name, attr_value in param_attr[name].items():
...
def setup_model_and_optimizer(
    ...
    if args.use_torch_compile:
        if args.use_regional_compilation:
            ...
            model[0] = torch.compile(
                model[0], backend='hpu_backend', dynamic=args.torch_compile_dynamic
            )
    ...
    if args.ckpt_convert_format is not None:
        ...
        assert is_real_cuda_device_available() or get_checkpoint_version() > 3.0 or mpu.get_pipeline_model_parallel_world_size() == 1, "Checkpoint conversion with PP>1 is not supported up to checkpoint version 3.0."
...
def train_step(forward_step_func, data_iterator, model, optimizer, opt_param_scheduler, config):
    ...
    while rerun_state_machine.should_run_forward_backward(data_iterator):
        ...
        losses_reduced = forward_backward_func(
            ...
            forward_only=False,
            compile_from_sec_mini_batch=(
                False
                if args.compile_from_sec_mini_batch is not None and args.curr_iteration > 0
                else args.compile_from_sec_mini_batch
            ),
    ...
    assert grad_norm > 0.0 and grad_norm != float("inf"), f"{grad_norm=}"
...
def training_log(
    ...
    if args.num_experts is not None:
        ...
        if args.moe_token_distribution_logging:
            report_token_distribution = (
                iteration % args.moe_token_distribution_logging_interval == 0
            )
            console_log = track_moe_token_distribution_metrics(
                report_token_distribution, iteration, writer
            )
            if console_log:
    ...
    if args.attention_z_loss_coeff > 0.0:
        ...
        track_attention_z_loss_metrics(
            z_loss_scale,
            iteration,
            writer,
            wandb_writer,
...
def checkpoint_and_decide_exit(
    ...
    if found_kill_switch(args):
        ...
        torch.distributed.barrier()
        print_datetime(
            f"Detected kill switch at {args.kill_switch_file}, " f"iteration={iteration}. Exiting"
        )
        return True

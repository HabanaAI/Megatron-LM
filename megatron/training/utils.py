...
try:
    ...
    ALL_MODULE_WRAPPER_CLASSNAMES = (DDP, torch_FSDP, custom_FSDP, Float16Module, torch._dynamo.eval_frame.OptimizedModule)
except ImportError:
    ALL_MODULE_WRAPPER_CLASSNAMES = (DDP, custom_FSDP, Float16Module, torch._dynamo.eval_frame.OptimizedModule)
...
def compile_methods(model):
    """Compile specific methods of the model if they exist."""
    compiled_methods = ['output_layer_and_loss']
    for method_name in compiled_methods:
        method = getattr(model, method_name, None)
        if method is not None:
            compiled_method = torch.compile(method, backend='hpu_backend')
            setattr(model, method_name, compiled_method)


def compile_regions(model):
    """Recursively compile regions of the model."""
    if model.__class__.__name__ == 'GPTModel':
        compile_methods(model)
    ...
    if isinstance(model, torch.nn.ModuleList):
        ...
        if model._modules:  # If model has submodules, recurse and reassign
            ...
            if isinstance(model, torch.nn.Module):
                model = model.compile(backend='hpu_backend')
            else:

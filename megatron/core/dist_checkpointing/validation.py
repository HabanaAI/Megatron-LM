...
def _validate_common_state_dict(common_state_dict: CommonStateDict) -> None:
    ...
    if rank == 0:
        ...
        for rank, rank_state_dict in enumerate(other_rank_state_dicts[1:], 1):
            ...
            skip_warning_for_key(only_left, main_rank_state_dict, 'step')
            skip_warning_for_key(only_right, rank_state_dict, 'step')
...
def skip_warning_for_key(dict_diff, state_dict, key):
    """Ignore non-relevant warnings during common state_dict validation.

    Args:
        dict_diff: Differences between the common state dictionaries of two ranks.
        state_dict: The state dictionary from one of the ranks.
        key: The specific key to skip during validation.
    """

    if len(dict_diff) == 0:
        return

    for keys in list(dict_diff):
        value = reduce(operator.getitem, keys, state_dict)
        if key in value:
            dict_diff.remove(keys)

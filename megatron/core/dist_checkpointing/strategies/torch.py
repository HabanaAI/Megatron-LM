    ...
    for sh in sh_ten.local_shards():
        ...
        if mcore_sh_ten.flattened_range is not None:
            ...
            for _ in range(mcore_sh_ten.prepend_axis_num):
                ten = ten.squeeze(0)
        # Below avoids having `exp_avg` and `exp_avg_sq` opimizer states as views.
        if is_lazy_mode() and not is_real_cuda_device_available() and 'exp_avg' in sh_ten.key:
            ret_tensors.append(ten.clone())
            del ten
        else:

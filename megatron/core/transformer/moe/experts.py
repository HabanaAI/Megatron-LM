...
class IntelFP8DynamicMLP(MegatronModule):
    ...
    weight tensors and different activation functions: silu, gelu, relu. This class is
    specialized for FP8 computations.
    """
    ...
    def __init__(
        ...
        super().__init__(config=config)
        assert submodules.linear_fc1 is None and submodules.linear_fc2 is None
        self.dynamic_mlp = build_module(submodules.linear_fc12, num_local_experts, config)

    def forward(
        self, hidden_states: torch.Tensor, router_weights: torch.Tensor, indices: torch.Tensor
    ):
        """Forward step of the IntelFP8DynamicMLP."""
        return self.dynamic_mlp(hidden_states, router_weights, indices)
